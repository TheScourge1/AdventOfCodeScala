...
trait Day extends App {
  ...
  case class TestCase(input: String,expectedResult: String,params :List[String]= List[String]()) {
    def eval(f: (String,List[String]) => String): Either[String,Boolean] = {
      ...
      if(res.equalsIgnoreCase(expectedResult)) Right(true)
      else Left("Expected: "+expectedResult + " but found "+res)
    }
  }
  ...
  override def main(args: Array[String]) = {
    ...
    if(testSetA == null) println("No testcases for solution A")
    ...
    if(testSetB == null) println("No testcases For solution B")
