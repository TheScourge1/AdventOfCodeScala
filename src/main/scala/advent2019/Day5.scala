...
object Day5 extends Day(5){
  ...
  override def solutionA(input: List[String], params: List[String]) = {
    ...
    OpcodeProcessor.processDay5OppCode(program,0,params(0).toInt,List())._2.takeRight(1).head
  }

  override def solutionB(input: List[String], params: List[String]) = {
    val program = input(0).split(",").map(s => s.toInt)
    OpcodeProcessor.processDay5OppCode(program,0,params(0).toInt,List())._2.head
  }
}
